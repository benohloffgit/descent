...
public sealed class CCText : MonoBehaviour {
	...
	public Vector3 lastCaret; // Added by Ben Ohloff
	...
	private void UpdateFromString () {
		...
		if(textLength == 0){
			HideSprites(0);
			usedSpriteCount = 0;
			lastCaret = transform.TransformPoint(Vector3.zero);
			return;
		}
		...
		while(true){
			if(c <= ' '){
				...
				if(++textIndex == textLength){
					...
					metaData.z = 0f;
					vertices[vertexIndex + 1] = metaData;
					caret.x += fontChar.AdvanceWithKerning(c); // Added by Ben Ohloff
					break;
				}
				c = text[textIndex];
				if(c == '\n'){
		...
		usedSpriteCount = nonWhiteSpaceCount;
//		Debug.Log ("length " + textLength + " " + caret.x);
		lastCaret = transform.TransformPoint(caret); // Added by Ben Ohloff
	}
