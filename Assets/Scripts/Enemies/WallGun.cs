...
public class WallGun : MonoBehaviour {
	...
	void Update() {
		Vector3 toShip = (play.GetShipPosition() - barrelAnchor.position).normalized;
		Vector3 rotateTo = Vector3.RotateTowards(barrelAnchor.forward, toShip, 0.01f, 0.01f);
...
//			barrelAnchor.LookAt(barrelAnchor.TransformPoint(rotateTo));
	...
	private void Shoot() {
		...
		Vector3 bulletDirection = barrelAnchor.forward * GUN_BULLET_SPEED;
